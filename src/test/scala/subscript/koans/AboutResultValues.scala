...
class AboutResultValues extends KoanSuite {
  ...
  koan(1)(
    """Scripts may have result values (similar to values returned from
  ...
  koan(2)(
    """You can specify that the script's result value should be that
  ...
  koan(3)(
    """If there's only one operand capable of having a result value
  ...
  koan(4)(
    """Each subsequent caret `^` overwrites script's result value."""
  ) {
    script s = {!1!}^ {!2!}^
    test(1) {
      runScript(s).$ shouldBe __
